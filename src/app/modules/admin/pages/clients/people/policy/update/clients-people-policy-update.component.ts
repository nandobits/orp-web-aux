...
@Component({
  selector: 'app-clients-people-policy-update',
  templateUrl: './clients-people-policy-update.component.html',
  styleUrls: ['./clients-people-policy-update.component.scss']
})
export class ClientsPeoplePolicyUpdateComponent implements OnInit {
    ...
    public onSavePolicy(){
        ...
        this.quotationService.updateQuotationData(this.policyId ,this.quotation).then( (res: any) => {
            this.policyService.updatePolicy(this.quotationCostId, this.policy).then( (res: any) => {
                if(res.error){
                    alert(res.message);
                }else{
