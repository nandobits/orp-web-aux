...
@Component({
  selector: 'app-clients-people-policy-cancel',
  templateUrl: './clients-people-policy-cancel.component.html',
  styleUrls: ['./clients-people-policy-cancel.component.scss']
})
export class ClientsPeoplePolicyCancelComponent implements OnInit {
    ...
    public onCancelPolicy(){
        let data = {
            reason_cancellation: this.policy.reason_cancellation
        }
        this.policyService.cancelPolicy(this.quotationCostId, data).then( (res: any) => {
            if(res.error){
                alert(res.message);
            }else{
